...
<div class="row">
<?php 
  $sql_data="SELECT DISTINCT a.*,b.nip FROM topik_forum a, f_mapel b WHERE a.id_mapel=b.id_mapel AND b.nip='$_SESSION[id_user]' ORDER BY a.id DESC";
        ...
        <table class="table table-striped" id="table_1">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                <h5 class="font-weight-bold text-primary"><b><small>[<?= $kat['nm_kategori'];?>] </small></b> <a href="?module=ug_forum&act=read&id=<?= $r['id'];?>"> <?=$r['judul_topik'];?></a></h5>
              ...
              <td><a href="?module=ug_forum&act=read&id=<?= $r['id'];?>" class="btn-sm btn-primary"><i class="fas fa-eye"></i></a></td>
            </tr>
            <?php $no++; } ?>
          </tbody>
        </table>
...
<div class="modal fade" id="tambah_data"  role="dialog" >
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <form action="?module=ug_forum&act=save" method="POST" role="form" enctype="multipart/form-data">
          ...
          <div class="form-group">
           ...
           <div class="form-group">
              <label for="">Isi Bahasan</label>
                <textarea class="editor_soal" name="isi_topik"  placeholder="Place some text here"></textarea>
           </div>
      ...
      <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
        ...
        <div class="dropdown no-arrow">
          <?php if ($s['id_user']==$_SESSION['id_user']) { ?>
            ...
            <a href="javascript:confirmdelete('?module=ug_forum&act=save&post=hapus_topik&id=<?php echo $s['id'];?>')" class="btn-sm btn-danger"><i class="fas fa-trash"></i></a>   
            <?php } else {} ?> 
            <a href="?module=ug_forum" class="btn-sm btn-secondary"><i class="fas fa-arrow-alt-circle-left"></i> Back</a>  
        </div>
      </div>
    </div>
  </div>
      ...
      <form action="?module=ug_forum&act=save" method="POST" role="form" enctype="multipart/form-data">
        ...
        ?>
          <div class="form-group">
           ...
           <div class="form-group">
              <label for="">Isi Bahasan</label>
                <textarea class="editor_soal" name="isi_topik"  placeholder="Place some text here"><?=$s['isi_topik'];?></textarea>
           </div>
...
<div class="row">
  <div class="col-md-8">
    <div class="card mb-4 py-3 border-bottom-primary bg-light">
      ...
      <div class="card-body">
        <table class="table table-striped" id="table_2"  width="100%" cellspacing="0" cellpadding="0">
          ...
          ?>
              <tr>
                ...
                <td>
                  <span class="text-primary"><?= $nama_org.' <small>('.$jabatan_org.')</small>';?></span><br><small><?= tgl_indo($kom['tgl_post_komentar']);?></small><br><?= $kom['isi_komentar'];?>
                </td>
                <td class="text-right">
                    <?php if ($kom['id_user']==$_SESSION['id_user']) { ?>
                    ...
                    <a href="javascript:confirmdelete('?module=ug_forum&act=save&post=hapus_komentar&id=<?= $kom['id'];?>&id_topik=<?=$id_topik;?>')" class="btn-sm btn-danger"><i class="fas fa-trash"></i></a>    
                    <?php } else {} ?> 
                </td>
              </tr>
              <!-- Edit Komentar-->
                  ...
                  <div class="modal-content">
                    ...
                    <div class="modal-body">
                      ...
                      ?>
                        ...
                        <div class="form-group">
                          <input type="hidden" name="id" value="<?=$r_kom['id'];?>">
                          <input type="hidden" name="id_topik" value="<?=$id_topik;?>">
                          <label for="">Isi Komentar</label>
                          <textarea class="editor" name="isi_komentar"  placeholder="Place some text here"><?= $r_kom['isi_komentar'];?></textarea>
                        </div>
                      </div>
                      <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
        ...
        ?>
           ...
           <div class="form-group">
            ...
            <input type="hidden" name="id_user" value="<?= $_SESSION['id_user'];?>">
              <label for="">Isi Komentar</label>
                <textarea class="editor" name="isi_komentar"  placeholder="Place some text here"></textarea>
           </div>
