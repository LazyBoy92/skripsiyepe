<?php
  ...
  if($user['level']=="user_guru") {
    ...
    $user_2= mysqli_fetch_array(mysqli_query($koneksi,"SELECT foto FROM guru WHERE id='$id_siswa'"));
  } else {
    $dir="module/foto_siswa";
    $user_2= mysqli_fetch_array(mysqli_query($koneksi,"SELECT foto FROM siswa WHERE id='$id_siswa'"));
  }
  ...
  if($dari['level']=="user_siswa"){
    ...
    $dari_2= mysqli_fetch_array(mysqli_query($koneksi,"SELECT id,nama_lengkap,foto FROM siswa WHERE id='$id_siswa'"));
    $nama_lengkap=$dari_2['nama_lengkap'].' <i class="text-info">==Siswa==</i>';

  }else{
  
    $dir2="module/foto_pengajar";
    $dari_2 = mysqli_fetch_array(mysqli_query($koneksi,"SELECT id,nama_lengkap,foto FROM guru WHERE id='$id_siswa'"));

    $nama_lengkap=$dari_2['nama_lengkap'].' <i class="text-info">==Guru==</i>';
...
<script> 
  ...
  function ambilPesan(){ 
      ...
      if(ajaxku.readyState == 4 || ajaxku.readyState == 0){ 
          ajaxku.open("GET","module/view_chat/ambilchat.php?akhir="+pesanakhir+"&nama="+namaku+"&sid="+Math.random(),true); 
  ...
  function aturAmbilPesan(){ 
      if(ajaxku.readyState == 4){ 
          ...
          for(i=0;i<data.messages.pesan.length;i++){ 
            ...
            if(data.messages.pesan[i].ke == <?= $_SESSION['id_user']; ?>){
              $(function(){  
  ...
  function kirimPesan(){ 
    ...
    var ext = fileName.substring(fileName.lastIndexOf('.') + 1);
      ...
      if(pesannya != "" && fileName == ""){ 
          ajaxku.open("GET","module/view_chat/ambilchat.php?"+pesanakhir+"&nama="+namaku+"&pesan="+pesannya+"&sid="+Math.random(),true); 
          ajaxku.onreadystatechange = aturAmbilPesan; 
          ajaxku.send(null); 
  	...
  	<div class="col-md-10">
    	<div class="card shadow" >
      		<div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
          		<h6 class="m-0 font-weight-bold text-primary">Chat Dari : <?= $nama_lengkap;?> </h6>
      	...
      	<div class="card-footer">
						...
						<div class="form-group">
							...
							<div class="row">
                <input type="text" name="pesan" id="pesan" class="form-control ml-3 mt-2" style="width: 780px;"> &nbsp;
								
                <div class="tomUpload btn btn-info"><i class="fas fa-paperclip"></i><input class="upload" type="file" name="file" id="file"></div>&nbsp;
                <button type="submit" class="tomUpload btn btn-success  " id="kirim"><i class="fas fa-paper-plane"></i></button>
							</div> 
						</div>
						<div class="form-group">
