...
header('location:../index.php');
      ...
      if($pg>0 AND $es>0) {
            $sql_data=mysqli_query($koneksi,"SELECT DISTINCT id_ujian,id_siswa,nilai FROM nilai WHERE id_ujian='$id_ujian' ");
      }
          ...
          <table class="table table-bordered table-striped" id="table_1"  width="100%" cellspacing="0" cellpadding="0">
            ...
            <tbody>
              ...
              while($row=mysqli_fetch_array($sql_data)) {
                ...
                $row2 = mysqli_fetch_array(mysqli_query($koneksi,"SELECT a.nama_lengkap,a.id, b.id_mapel, c.nis FROM siswa a, f_mapel b, f_kelas c WHERE a.nis=c.nis AND b.id_kelas = c.id_kelas AND a.id='$row[id_siswa]'"));
                //echo "SELECT a.nama_lengkap,a.id, b.id_mapel, c.nis FROM siswa a, f_mapel b, f_kelas c WHERE a.nis=c.nis AND b.id_kelas = c.id_kelas AND b.nip='$_SESSION[id_user]'";
                $row3 = mysqli_fetch_array(mysqli_query($koneksi,"SELECT a.nama_kelas,b.id_kelas FROM m_kelas a,f_kelas b WHERE a.id_kelas=b.id_kelas AND b.nis='$row2[nis]'"));
                $row4 = mysqli_fetch_array(mysqli_query($koneksi,"SELECT sum(nilai) as jum FROM nilai_esay WHERE id_ujian='$id_ujian' AND id_siswa='$row[id_siswa]' "));
                  ...
                  if($row4['jum']==0){
                        $n_esay = 'Belum Koreksi <br> <a href="?module=f_ujian&act=koreksi&ujian='.$id_ujian.'&siswa='.$row2['id'].'" class="btn-sm btn-primary"><i class="fas fa-exclamation-circle"></i>Koreksi</a>';
                  }
                  else{
                        $n_esay = $r['bobot_esay']/100*$row4['jum'].'<br><a href="?module=f_ujian&act=koreksi&ujian='.$id_ujian.'&siswa='.$row2['id'].'" class="btn-sm btn-primary"><i class="fas fa-exclamation-circle"></i><small>Recorect</small></a>';
                  }
              ...
              <tr>
                ...
                <td align="center"><?=$n_pg;?><br> <a href="?module=f_ujian&act=analisa_pg&ujian=<?=$id_ujian;?>&siswa=<?=$row2['id'];?>" class="btn-sm btn-primary"><i class="fas fa-check"></i><small>Analisa</small></a></td>
              ...
              <div class="modal fade " id="reset_pg<?= $row['id_siswa'];?>"  role="dialog" >
                <div class="modal-dialog modal-sm" role="document">
                  ...
                  <form method="POST" action="?module=f_ujian&act=reset_pg" enctype="multipart/form-data">
              ...
              <div class="modal fade " id="reset_es<?= $row['id_siswa'];?>"  role="dialog" >
                <div class="modal-dialog modal-sm" role="document">
                  <div class="modal-content bg-gradient-warning">
                  <form method="POST" action="?module=f_ujian&act=reset_essay" enctype="multipart/form-data">
                        <div class="modal-header">
...
<div class="modal fade" id="cetak"  role="dialog" >
  <div class="modal-dialog modal-md" role="document">
    ...
    <form method="POST" action="module/f_ujian/cetak_laporan.php" enctype="multipart/form-data" target="_blank">
      <div class="modal-header">
        <h4 class="modal-title" >Cetak Laporan Nilai</h4>
      </div>
      <?php 
        $nip_gr=mysqli_fetch_array(mysqli_query($koneksi,"SELECT username FROM user WHERE id_user='$r[pembuat]'"));
		//echo "SELECT username FROM user WHERE id_user='$r[pembuat]'";
		//echo "SELECT a.nama_kelas, b.id_kelas FROM m_kelas a, kelas_ujian b WHERE a.id_kelas=b.id_kelas AND b.id='$id_ujian'";
       ?>
