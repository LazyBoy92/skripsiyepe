        ...
        <div class="table-responsive">
          <table class="table table-bordered table-striped" id="table_1"  width="100%" cellspacing="0" cellpadding="0">
            ...
            <tbody style="font-size: 12px;">
              <?php 
                $sql_data = mysqli_query($koneksi,"SELECT a.*,b.nama_mapel FROM topik_ujian a, m_mapel b WHERE a.id_mapel = b.id_mapel AND a.tgl_buat BETWEEN '$thn_lalu' AND '$thn_skrg' ORDER BY a.id DESC");
                $no=1;
                while($r=mysqli_fetch_array($sql_data)){
                  
                  if($r['pembuat'] =='1') { 
                    $pmb = mysqli_fetch_array(mysqli_query($koneksi,"SELECT nama_lengkap, level FROM user WHERE id_user = '$r[pembuat]'"));
                    $nama_pembuat = $pmb['nama_lengkap'];
                    $jab_pembuat  = $pmb['level']; 
                  }
                  elseif($r['pembuat'] == '2') {
                    $pmb = mysqli_fetch_array(mysqli_query($koneksi,"SELECT nama_lengkap, level FROM user WHERE id_user = '$r[pembuat]'"));
                    $nama_pembuat = $pmb['nama_lengkap'];
                    ...
                    $pmb = mysqli_fetch_array(mysqli_query($koneksi,"SELECT nama_lengkap, jabatan FROM guru WHERE id = '$r[pembuat]'"));
                    $nama_pembuat = $pmb['nama_lengkap'];
                    $jab_pembuat  = $pmb['jabatan'];  

                  }

                   //echo "SELECT nama_lengkap, level FROM user WHERE id_user = '$r[pembuat]'";
                  
              ?>
                <tr>
                  <td><?= $no;?></td>
                  <td><?= $r['judul'];?><br>
                      Pembuat : <?= $nama_pembuat;?><br>
                      Jabatan : <?= $jab_pembuat;?><br>
                      Mapel   : <?= $r['nama_mapel'];?>
                      <?= $a;?>
                  </td>
                  <td>
                  ...
                  <td align="center"><a href="?module=f_ujian&act=detail_soal&id=<?=$r['id'];?>" class="btn-sm btn-info"><i class="fas fa-search"></i></a></td>
                  <td align="center"><a href="?module=f_ujian&act=detail_peserta&id=<?=$r['id'];?>" class="btn-sm btn-success"><i class="fas fa-graduation-cap"></i></a></td>
                  ...
                  <td align="center"><a href="javascript:confirmdelete('?module=f_ujian&act=hapus_topik&id=<?php echo $r['id'];?>')" class="btn-sm btn-danger"><i class="fas fa-trash"></i></a></td>
                  </tr>
                  ...
                  <div class="modal fade" id="edit<?php echo $r['id'];?>"  role="dialog" >
                    <div class="modal-dialog modal-lg" role="document">
                      <div class="modal-content">
                        ...
                        <form action="?module=f_ujian&act=save" method="POST" role="form" enctype="multipart/form-data">
                          <?php 
                        ...
                        <div class="modal-body">
                          ...
                          <div class="form-group">
                           ...
                           <div class="form-group">
                              ...
                              <?php $jumn=mysqli_num_rows(mysqli_query($koneksi,"SELECT * FROM kelas_ujian WHERE id ='$e[id]'"));?>
                                <select name="id_kelas[]" class="form-control select2bs4" multiple="multiple">
                                    ...
                                    ?>
                                    <option value="<?php echo $p['id_kelas']; ?>" selected> <?php echo $ng['nama_kelas']; }?></option>
                                    <?php
        ...
        <div class="form-group">
         ...
         <div class="form-group">
            ...
            <select name="id_mapel" class="form-control select2bs4" required="required">
              ...
              <?php
              $kl=mysqli_query($koneksi,"SELECT * FROM m_mapel  ORDER BY id_mapel ASC");
              while($dkl=mysqli_fetch_array($kl)) {
              ?>
              <option value="<?php echo $dkl['id_mapel'];?>"><?php echo $dkl['nama_mapel'];?></option>
              <?php } ?>
         ...
         <div class="form-group">
            ...
            <select name="id_kelas[]" class="form-control select2bs4" required="required" multiple="multiple">
              ...
              <?php
              $kls=mysqli_query($koneksi,"SELECT * FROM m_kelas ORDER BY id_kelas ASC");
              while($dkls=mysqli_fetch_array($kls)) {
              ?>
              <option value="<?php echo $dkls['id_kelas'];?>"><?php echo $dkls['nama_kelas'];?></option>
              <?php } ?>
      ...
      <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
          <h6 class="m-0 font-weight-bold text-primary">Detail Soal Ujian</h6>
           <div class="dropdown no-arrow">
            ...
            <a href="?module=f_ujian" class="btn-sm btn-warning"><i class="fas fa-arrow-alt-circle-left"></i> Back</a>
          </div>
      </div>
      <div class="card-body">
        <div class="table-responsive">
          ...
          <tfoot>
            <tr>
              <td style="text-align: right;">
                    <a href="?module=f_ujian&act=essay&id=<?=$s['id'];?>" class="btn btn-success btn-icon-split"><span class="icon text-white-50"><i class="fas fa-search"></i></span><span class="text">Soal Essay</span></a>
                  </td>
                  <td>
                    <a href="?module=f_ujian&act=pil_ganda&id=<?=$s['id'];?>" class="btn btn-primary btn-icon-split"><span class="text">Pilihan Ganda</span><span class="icon text-white-50"><i class="fas fa-search"></i></span></a>
                  </td>
              </tr>
          </tfoot>
        </table>
    ...
    <div class="card shadow">
      <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
          <h6 class="m-0 font-weight-bold text-primary">List Soal Essay</h6>
           <div class="dropdown no-arrow">
            <a class="dropdown-toggle" href="#" role="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"></a>
            <a href="?module=f_ujian&act=detail_soal&id=<?= $_GET['id'];?>" class="btn-sm btn-warning"><i class="fas fa-arrow-alt-circle-left"></i> Back</a>
          ...
          <table class="table table-bordered table-striped" id="table_1">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td align="center"> <a href="javascript:confirmdelete('?module=f_ujian&act=hapus&id=<?= $t['id_soal'];?>&id_tujian=<?= $t['id_tujian'];?>')" class="btn-sm btn-danger"><i class="fas fa-trash"></i></a></td>
              </tr>
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <input type="hidden" class="form-control" name="id_tujian" value="<?= $_GET['id'];?>">
      <?php 
        $sql_bank_essay = mysqli_query($koneksi,"SELECT * FROM bank_esay ");
        $no = 1;
      ?>
          ...
          <table class="table table-bordered table-striped" id="table_1" style="font-size: 13px;">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td align="center"><a href="javascript:confirmdelete('?module=f_ujian&act=hapuspg&id=<?= $t['id_soalpg'];?>&id_tujian=<?= $t['id_tujian'];?>')" class="btn-sm btn-danger"><i class="fas fa-trash"></i></a></td>
              </tr>
  ...
  <div class="modal-dialog modal-xl" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <input type="hidden" class="form-control" name="id_tujian" value="<?= $_GET['id'];?>">
      <?php 
        $sql_bank_pg = mysqli_query($koneksi,"SELECT * FROM bank_pilganda ");
        $no = 1;
      ?>
      <table class="table table-striped" id="tab_bank1" >
        <thead>
        ...
        <tbody>
          ...
          foreach ($sql_bank_pg as $rp) {
            if($rp['kunci']==1){$kunci = 'A';} elseif($rp['kunci']==2){$kunci = 'B';} elseif($rp['kunci']==3){$kunci = 'C';} elseif($rp['kunci']==4){$kunci = 'D';} else{$kunci = 'E';}
...
case "hapus_topik":
  ...
  save_alert('save','Soal di Hapus');
  htmlRedirect('media.php?module='.$module.'&act=f_ujian');
break;
